...
import moveit_motion.diffusion_policy_cam.MSEC.data_processing as dproc
import moveit_motion.diffusion_policy_cam.MSEC.model as md
...
def predicted_action(obs_deque, statistics, obs_horizon,
    ...
    nagent_poses = dproc.normalize_data(agent_poses, stats=stats['agent_pos'])
...
def _model_initalization(action_dim, obs_dim, small_obs_dim, obs_horizon, pred_horizon, size, num_epochs, len_dataloader, num_diffusion_iters) -> nn.Module:

    # create vision encoder
    # construct ResNet18 encoder
    # if you have multiple camera views, use seperate encoder weights for each view.
    vision_encoder = ve.get_resnet('resnet18')

    # IMPORTANT!
    # replace all BatchNorm with GroupNorm to work with EMA
    # performance will tank if you forget to do this!
    vision_encoder = ve.replace_bn_with_gn(vision_encoder)
    ...
    image_size = (1, obs_horizon,3) + size
    ...
    with torch.no_grad():
        ...
        agent_pos = torch.zeros((1, obs_horizon, small_obs_dim))
        # vision encoder
    ...
    return ema_nets, noise_scheduler, device, ema, optimizer, lr_scheduler
...
def move_robot_get_obs_thread(action, obs_deque, kb, kb2, pipeline, ati_client):
...
def moving_robot_joint_ptp_obs(action, obs_deque, kb, ati_client, pipeline, size):
    ...
    for joint_values in action:
        ...
        color_frame = frames.get_color_frame()
        image_resized = np.asanyarray(color_frame.get_data(), dtype=np.float32)
        image_observation = cv2.resize(image_resized, size)
        cv2.imshow('RealSense', image_observation)
...
def moving_robot_cartesian_ptp_obs(action, obs_deque, kb, ati_client, pipeline, size):
    ...
    for cartesian_values in action_robot:
        ...
        if _Txyz_Rxyz is not None and force_observations.done() and image_observation is not None:
...
def moving_robot_joint_ptp_obs_with_box(action, obs_deque, kb, ati_client, pipeline, size, box_observation):
...
def moving_robot_cartesian_ptp_obs_with_box(action, obs_deque, kb, ati_client, pipeline, size, box_observation):
...
if __name__ == "__main__":
    rclpy.init()
    ############################################################33
    checkpoint_path = '/home/cam/Downloads/checkpoints/checkpoint_vision_model_basic_resize_96_96_epoch_199.pth' 
    size = (96, 96)
    box_observation = [-1.9180826862850697, -0.9203766683060719, -0.7417574885955796, 1.2914243302049004, -2.38082111368872, -0.10695093811143339, -0.1087187311689809]
    ...
    ema_net, noise_scheduler, device, ema, optimizer, lr_scheduler = _model_initalization(action_dim, obs_dim, small_obs_dim, obs_horizon, pred_horizon, size, num_epochs, len_dataloader, num_diffusion_iters)
    
    ema_net['noise_pred_net'].load_state_dict(checkpoint['model_state_dict'])
    # optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    # lr_scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
    ema.load_state_dict(checkpoint['ema_state_dict'])
    ema_net['vision_encoder'].load_state_dict(checkpoint['vision_state_dict'])
    ...
    while True:
        ...
        obs_deque = moving_robot_joint_ptp_obs(action, obs_deque, kb, ati_client, pipeline, size)
        # obs_deque = moving_robot_cartesian_ptp_obs(action, obs_deque, kb, ati_client, pipeline, size)
        # obs_deque = moving_robot_joint_ptp_obs_with_box(action, obs_deque, kb, ati_client, pipeline, size, box_Txyz_Rxyz)
        # obs_deque = moving_robot_cartesian_ptp_obs_with_box(action, obs_deque, kb, ati_client, pipeline, size, box_Txyz_Rxyz)
        action =  predicted_action(obs_deque,  statistics, obs_horizon,
                            pred_horizon, action_horizon, action_dim, noise_scheduler, num_diffusion_iters,
                            ema_net, device)#
