...
def main(save_dir, traj_num):
    ...
    config1.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, 30)  # Adjust stream parameters as neededo
    # Start the pipelines
    ...
    with keyboard.Listener(on_press=on_press) as listener:
        ...
        while True:
            ...
            if is_writing:
                ...
                depth_observation1 = np.asanyarray(depth_frame1.get_data())
                ...
                if force_observations.done() and image_observation1 is not None: # and image_observation2 is not None:
                    ...
                    # Save the image as a PNG fileo
                    # print(f"Saving image as: {image_filename}")
                    cv2.imwrite(image_filename, image_observation1)
                    # Save the depth image if needed
                    depth_filename = os.path.join(image_folder, f"{row_index + 1}_depth.png")
                    cv2.imwrite(depth_filename, depth_observation1)
...
if __name__ == '__main__':
    main('no-sync/rgbd_force_motive_data', 1)
